...
public class Trapper implements MenuItem {
    @Override
    public SwagCraftKit getKit() {
        return SwagCraftKit.TRAPPER;
    }
    ...
    public ItemStack getItem() {
        ...
        return ItemStack.builder(Material.NETHER_STAR).displayName(holder.getName().decoration(TextDecoration.ITALIC, false)).lore(lore).meta(mb -> mb.hideFlag(ItemHideFlag.HIDE_ENCHANTS).hideFlag(ItemHideFlag.HIDE_ATTRIBUTES)).build();
    }

    private MenuItemHolder getMiniMessage() {
        MiniMessage mm = MiniMessage.miniMessage();
        Component name = mm.deserialize("<gray>Trapper</gray>");
        List<Component> lore = new ArrayList<>();
        ...
        lore.add(mm.deserialize("<blue>lost in time.</blue>"));
        lore.add(Component.empty());
        lore.add(mm.deserialize("<blue>As compensation, you get</blue>"));
        lore.add(mm.deserialize("<blue>a stick.</blue>"));
